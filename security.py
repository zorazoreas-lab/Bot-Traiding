...
from app.services.binance_service import BinanceAPIError
...
async def run_bot_once(db: Session, bot: Bot) -> dict:
    bundle = get_binance_credentials(db, bot.user_id)
    if not bundle:
        log_safety_event(db, "NO_API_KEY", "No Binance API key found in ENV or database", "Bot skipped", bot.id)
        return {"ok": False, "message": "No Binance API key found in ENV or database"}
