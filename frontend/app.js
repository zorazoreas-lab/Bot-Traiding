...
function setText(id, data) { document.getElementById(id).textContent = typeof data === 'string' ? data : pretty(data); }
...
async function connectBinance() {
  try {
    const data = await api('/api/binance/connect', {
      method: 'POST',
      body: JSON.stringify({ api_key: apiKey.value, secret_key: secretKey.value, is_testnet: isTestnet.checked })
    });
